...
export default function VivuplanPremiumApp() {
  ...
  // ====== ADDED: get current user id (from login) + session id generator ======
  ...
  // CHỈ dùng localStorage user_id nếu nó không giống email
  ...
  if (fromStorage && !looksLikeEmail(fromStorage)) return String(fromStorage);
  return "1";
};
  ...
  const getBannerPhoto = (name: string) => {
    return `https://images.unsplash.com/photo-1506461883276-594a12b11cf3?auto=format&fit=crop&q=60&w=1200&sig=${encodeURIComponent(
      name
    )}`;
  };
  ...
  try {
    ...
    const resH = await fetch(`${API_BASE}/conversation/history/${uid}?page=1&page_size=10`);
    const dataH = await resH.json();

    if (dataH?.data) {
      const sorted = [...dataH.data].sort((a: any, b: any) => {
  ...
  const handleSelectChat = async (sid: string) => {
    try {
      setSelectedDayIdx(null); // ====== ADDED: avoid overlay staying open when switching chats ======
      setActiveId(sid);
  ...
  const handleNewChat = () => {
    // ====== CHANGED: create a new session id immediately so "new chat" actually works ======
  ...
  useEffect(() => {
    ...
    const loadInitial = async () => {
      try {
        // ====== CHANGED (added): use current user id if available ======
        const uid = getCurrentUserId();
        const resH = await fetch(`${API_BASE}/conversation/history/${uid}?page=1&page_size=1`);
        const dataH = await resH.json();
        if (dataH?.data?.length) {
          handleSelectChat(dataH.data[0].session_id);
        } else {
          // if user has no history, start a new session id to allow sending immediately
          setActiveId(newSessionId());
        }
      } catch (e) {
  ...
  const places: UiPlace[] = useMemo(() => {
    ...
    itineraryData.itinerary.forEach((day: any) => {
      const items = [
        ...(day.attraction_recommendations || []),
        ...(day.restaurant_recommendations || []),
      ];
  ...
  // ====== CHANGED: real send -> call POST /v1/conversation/response ======
  const handleSend = async () => {
    ...
    try {
      ...
      // API response schema is unclear in your Swagger (could be string or JSON)
      ...
      if (aiText && aiText.trim()) {
        ...
        // fallback: re-fetch conversation to update messages + itinerary/hotel
        const resD = await fetch(`${API_BASE}/conversation/${sid}`);
        const dataD = await resD.json();
        setMessages(
          (dataD?.messages || []).map((m: any) => ({
      ...
      setMessages((prev) => [
        ...prev,
        {
          role: "ai",
          content:
            "Không nhận được phản hồi từ server. Bạn mở DevTools → Network để xem lỗi (422/500) nhé.",
        },
      ]);
    }
  };
  ...
  return (
    <div className="fixed inset-0 w-screen h-screen bg-[#FDFDFD] text-slate-900 font-sans flex flex-col md:flex-row overflow-hidden">
        ...
        <div className="h-32 border-b border-slate-50 overflow-y-auto p-4 space-y-1 shrink-0 bg-slate-50/30 mt-2 mx-4 rounded-xl">
          ...
          {chatHistory.map((chat) => (
            <button
              ...
              className={`w-full text-left p-2 rounded-lg text-[11px] truncate flex items-center gap-2 transition-colors ${
                activeId === chat.session_id
                  ? "bg-blue-600 text-white font-bold"
                  : "hover:bg-white text-slate-600"
              }`}
            >
              <MessageSquare size={12} className="shrink-0" /> Trip{" "}
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4 bg-slate-50/30 custom-scrollbar scroll-smooth">
          {messages.map((m, i) => (
            <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
              <div
                className={`max-w-[85%] p-4 rounded-2xl text-[12px] shadow-sm ${
                  m.role === "user"
        ...
        <div className="p-4 bg-white border-t border-slate-100 shrink-0">
          <div className="relative group flex items-center gap-2">
            <textarea
              ...
              className="flex-1 bg-slate-50 border border-slate-100 rounded-xl p-3 text-xs h-11 resize-none outline-none focus:ring-2 focus:ring-blue-100 transition-all font-medium"
              placeholder="Nhắn tin cho Vivuplan..."
            />
            <button
              onClick={handleSend}
              className="w-11 h-11 bg-black text-white rounded-xl flex items-center justify-center hover:bg-[#0056D2] transition-colors"
            >
              <Send size={16} />
            </button>
          </div>
      ...
      <div className="flex-1 h-full flex flex-col overflow-hidden relative bg-[#F8FAFB]">
        <main
          className={`flex-1 overflow-y-auto custom-scrollbar p-4 md:p-10 space-y-10 md:space-y-12 pb-32 relative transition-all duration-700 ${
            selectedDayIdx !== null ? "md:pr-[450px]" : ""
          }`}
        >
            ...
            <img
              ...
              onError={(e) => {
                (e.target as HTMLImageElement).src =
                  "https://images.unsplash.com/photo-1599708153386-62bf3f035978?auto=format&fit=crop&w=1400";
              }}
            />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="md:col-span-2 bg-white p-2 rounded-[1.5rem] md:rounded-[2.5rem] shadow-xl border h-96 md:h-[500px] relative overflow-hidden group">
              ...
              <button
                ...
                className="absolute bottom-6 right-6 px-6 py-3 bg-white/95 backdrop-blur rounded-full text-[9px] md:text-[11px] font-black shadow-lg text-[#0056D2] flex items-center gap-2 uppercase tracking-widest hover:bg-[#0056D2] hover:text-white transition-all z-10 font-black italic"
              >
            ...
            <div className="bg-[#0056D2] p-8 rounded-[1.5rem] md:rounded-[2.5rem] shadow-xl text-white flex flex-col justify-center relative overflow-hidden group">
              ...
              <p className="text-[10px] font-black uppercase tracking-[0.3em] opacity-60 mb-2 font-black italic">
                Tổng dự chi chuyến đi
              </p>
              <h2 className="text-4xl font-black italic tracking-tighter mb-4">
                {totalCost.toLocaleString()}đ
              </h2>
              <div className="pt-4 border-t border-white/20">
                <p className="text-[11px] font-medium opacity-80 leading-relaxed italic font-bold">
                  Lịch trình tối ưu bởi AI
                </p>
              </div>
            </div>
          </div>
          ...
          <section className="space-y-6">
            <h2 className="text-base md:text-lg font-black italic tracking-widest text-[#0056D2] px-2 flex items-center gap-3 uppercase font-black italic">
              <Calendar size={20} /> LỘ TRÌNH CHI TIẾT
            </h2>
            <div className="space-y-4">
              {itineraryData?.itinerary?.map((day: any, idx: number) => (
                <div
                  ...
                  className="bg-white p-4 md:p-6 rounded-[1.5rem] md:rounded-[2.5rem] shadow-sm border border-slate-50 flex flex-col md:flex-row gap-4 md:gap-8 items-start md:items-center cursor-pointer hover:shadow-xl transition-all group"
                >
                  ...
                  <div className="w-full md:w-32 h-20 bg-slate-100 rounded-[1rem] flex items-center justify-center shrink-0 border border-slate-100 group-hover:bg-[#0056D2] transition-colors">
                    <p className="text-sm font-black text-slate-400 uppercase italic tracking-tighter group-hover:text-white">
                      Ngày {idx + 1}
                    </p>
                  </div>

                  <div className="flex-1">
                    <span className="text-[10px] font-bold text-cyan-800 block mb-1 border-l-3 border-cyan-500 pl-3 uppercase font-bold">
                      {day.date_}
                    </span>
                    <h3 className="text-base md:text-lg font-black italic uppercase group-hover:text-[#0056D2] transition-colors leading-tight font-black italic">
                      KHÁM PHÁ {day.location}
                    </h3>
                    <p className="text-[12px] text-slate-500 mt-2 line-clamp-2 md:line-clamp-1 italic font-medium">
                      "{day.morning}"
                    </p>
                  </div>
                  <div className="hidden md:flex w-10 h-10 bg-[#0056D2] rounded-full items-center justify-center text-white shadow-xl group-hover:scale-110 transition-all">
                    <ArrowRight size={20} />
                  </div>
                </div>
              ))}
          ...
          <section className="space-y-8 pt-10 border-t border-slate-100 pb-10">
            <h2 className="text-lg md:text-xl text-[#0056D2] px-2 italic uppercase tracking-tighter font-black font-black italic">
              KHÁCH SẠN ĐỀ XUẤT
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
              {hotelData?.recommended_hotels?.map((h: any, i: number) => (
                <div
                  key={i}
                  className="bg-white p-4 md:p-6 rounded-[2rem] md:rounded-[3rem] shadow-sm border border-slate-100 group hover:shadow-2xl transition-all flex flex-col"
                >
                  ...
                  <h4 className="text-base md:text-lg font-black mb-1 group-hover:text-[#0056D2] transition-colors uppercase italic text-slate-800 truncate font-black italic">
                    {h.hotel_id.slice(-5)} Luxury Hotel
                  </h4>
                  <p className="text-[11px] text-slate-600 italic line-clamp-2 mb-6 leading-relaxed normal-case">
                    {h.reasoning}
                  </p>
                  <div className="mt-auto flex justify-between items-center pt-5 border-t border-slate-100 font-black italic">
                    <div>
                      ...
                      <p className="text-xl font-black text-[#0056D2]">
                        {h.hotel_id.slice(0, 1)},500,000đ
                      </p>
                    </div>
                    <button className="bg-slate-900 text-white px-6 py-3 rounded-full text-[9px] font-black hover:bg-[#0056D2] transition-all uppercase italic shadow-lg">
                      ĐẶT PHÒNG
                    </button>
                  </div>
                </div>
        ...
        {selectedDayIdx !== null && itineraryData?.itinerary?.[selectedDayIdx] && (
          <aside className="hidden md:block fixed right-0 top-0 bottom-0 w-[450px] bg-white border-l border-slate-100 z-50 shadow-2xl animate-in slide-in-from-right duration-500">
            <PlacesMapPane
              places={places.filter((p) => p.day === itineraryData.itinerary[selectedDayIdx].date_)}
            />
            <div className="absolute top-10 left-10 bg-white/95 px-6 py-3 rounded-full shadow-2xl border border-slate-100 flex items-center gap-3 z-50 font-bold uppercase italic text-[10px] tracking-widest font-black italic">
              <div className="w-2 h-2 rounded-full bg-blue-600 animate-ping" /> BẢN ĐỒ NGÀY{" "}
              {selectedDayIdx + 1}
            </div>
          </aside>
        )}
          ...
          <div className="fixed inset-0 bg-white z-[120] md:z-[55] pt-16 md:pt-28 p-4 md:p-16 overflow-y-auto animate-in slide-in-from-bottom md:slide-in-from-right duration-500 md:left-[320px] md:right-[450px] border-r border-slate-100 shadow-inner">
            <div className="max-w-2xl mx-auto pb-48 font-black italic uppercase tracking-widest font-black italic">
              <button
                ...
                className="mb-8 flex items-center gap-2 text-[10px] font-black text-[#0056D2] border-2 border-blue-50 px-5 py-2.5 rounded-full bg-white hover:bg-blue-50 transition-all uppercase tracking-widest font-black italic"
              >
                <ArrowLeft size={14} /> QUAY LẠI LỘ TRÌNH
              </button>
              <div className="flex flex-col gap-2 mb-12">
                <h2 className="text-4xl md:text-6xl font-black text-[#0056D2] italic tracking-tighter uppercase opacity-90 leading-tight italic font-black italic">
                  NGÀY {selectedDayIdx + 1}
                </h2>
                <p className="text-base md:text-lg font-bold uppercase tracking-[0.2em] italic text-slate-400 font-black italic">
                  {itineraryData.itinerary[selectedDayIdx].location}
                </p>
              </div>
              <div className="space-y-12">
                <div className="space-y-5">
                  <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] px-6 italic border-l-4 border-cyan-500 font-black italic">
                  ...
                  <div className="space-y-4">
                    {["morning", "afternoon", "evening"].map((session, sIdx) => (
                      <div
                        key={sIdx}
                        className="flex gap-5 items-start p-6 bg-slate-50 rounded-[2rem] border border-transparent hover:border-slate-200 hover:bg-white transition-all font-black italic"
                      >
                        ...
                        <div>
                          <p className="text-[9px] font-black text-slate-400 uppercase italic mb-1 tracking-widest">
                            {session.toUpperCase()}
                          </p>
                          <p className="text-sm md:text-base font-bold italic text-slate-700 leading-relaxed normal-case font-black italic font-medium italic">
                ...
                <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100 flex flex-col md:flex-row justify-between items-center gap-6 font-black italic">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-blue-50 rounded-xl flex items-center justify-center text-[#0056D2]">
                      <Car size={24} />
                    </div>
                    <div>
                      <p className="text-[9px] font-black text-slate-300 italic uppercase">
                        DI CHUYỂN
                      </p>
                      <p className="text-sm font-bold text-slate-700 italic uppercase">
                        {itineraryData.itinerary[selectedDayIdx].transportation}
                      </p>
                    </div>
                  </div>
                  <div className="text-center md:text-right">
                    ...
                    <p className="text-3xl font-black text-[#0056D2] italic tracking-tighter">
                      {itineraryData.itinerary[selectedDayIdx].estimated_cost.max.toLocaleString()}đ
                    </p>
                  </div>
                </div>
                <div className="bg-[#0A0F1A] p-10 md:p-16 rounded-[4rem] text-white relative overflow-hidden shadow-2xl font-black italic">
                  ...
                  <h4 className="text-[10px] font-black text-[#4ECDC4] uppercase tracking-[0.5em] mb-10 italic flex items-center gap-4 relative z-10 font-black italic tracking-widest">
                    <Utensils size={20} /> ẨM THỰC ĐỊA PHƯƠNG
                  </h4>
                  <div className="grid grid-cols-1 gap-10 relative z-10">
                    {itineraryData.itinerary[selectedDayIdx].meals.map((meal: string, i: number) => {
                      const [time, dish] = meal.split(":");
                      return (
                        <div
                          key={i}
                          className="group pl-6 border-l border-white/10 hover:border-[#4ECDC4] transition-all font-black italic tracking-widest"
                        >
                          <span className="text-[9px] font-black text-slate-500 uppercase block mb-2 italic tracking-[0.2em] group-hover:text-[#4ECDC4]">
                            {time?.toUpperCase()}
                          </span>
                          <p className="text-base font-bold text-slate-100 italic tracking-tight uppercase">
                            {dish}
                          </p>
                        </div>
                      );
