...
export default function ChatboxPage() {
  ...
  useEffect(() => {
    ...
    const loadMessages = async () => {
      ...
      try {
        ...
        const data = await res.json();
        const msgs = (Array.isArray(data) ? data : data?.messages || []).map(
          (m: any) => ({
  ...
  // ✅ THÊM: Xoá 1 cuộc trò chuyện (UI)
  const deleteThread = (id: string) => {
    setThreads((prev) => {
      const remain = prev.filter((t) => t.id !== id);

      // nếu đang xoá đúng thread đang active thì nhảy sang thread khác
      if (activeId === id) {
  ...
  return (
    <div className="min-h-screen bg-[#f1f5f9] font-sans">
      ...
      <main className="mx-auto max-w-[1600px] px-8 py-8 grid grid-cols-12 gap-6 h-[calc(100vh-80px)]">
          ...
          <GlassCard className="p-4 h-full flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto mt-4 space-y-2 pr-2 custom-scrollbar">
              {threads.map((t) => (
                ...
                >
                  {/* Click để chọn thread */}
                  <button
                    onClick={() => setActiveId(t.id)}
                    className="w-full text-left p-4 pr-12 rounded-2xl"
                  >
                  ...
                  <button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      const ok = window.confirm("Xoá cuộc trò chuyện này?");
                      if (!ok) return;
                      deleteThread(t.id);
                    }}
                    className="
        ...
        <div
          className={`${
            resultsOpen ? "col-span-5" : "col-span-9"
              ...
              <div className="bg-white p-6 rounded-3xl shadow-2xl border border-slate-100 ring-1 ring-black/5">
                ...
                <div className="grid grid-cols-4 gap-3 mb-4">
                  ...
                  <InputBox
                    label="Đến"
                    value={config.to}
                    onChange={(v) => setConfig({ ...config, to: v })}
                  />
                  <InputBox
                    label="Ngày đi"
                    type="date"
                ...
                <div className="flex gap-3 items-end">
                  <div className="flex-1">
                    ...
                    <input
                      ...
                      className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-xs font-bold focus:border-cyan-500 outline-none transition-all"
                    />
                  </div>
                  <div className="flex gap-2">
                    <MiniInput
                      label="Khách"
                      value={config.guests}
                      onChange={(v) => setConfig({ ...config, guests: v })}
                    />
                    <MiniInput
                      label="Ngày"
                      value={config.days}
                  ...
                  <button
                    ...
                    className="h-[40px] px-6 bg-cyan-600 text-white rounded-xl font-black text-[10px] uppercase tracking-widest shadow-xl shadow-cyan-100 hover:bg-cyan-700 transition-all"
                  >
                    THIẾT KẾ
                  </button>
                </div>
            ...
            <div className="p-4 bg-white border-t flex items-center gap-3">
              ...
              <div className="flex-1 relative flex items-center bg-slate-50 rounded-2xl ring-1 ring-slate-200 focus-within:ring-cyan-500 transition-all">
                ...
                <button
                  onClick={() => sendText()}
                  disabled={!text.trim()}
                  className="absolute right-2 h-9 w-9 bg-cyan-600 text-white flex items-center justify-center rounded-xl hover:bg-cyan-700 transition-all"
                >
                  <svg
                    className="h-4 w-4"
                    fill="none"
          ...
          <div className="col-span-4 h-full">
            <GlassCard className="h-full flex flex-col overflow-hidden shadow-2xl">
              <div className="p-6 border-b flex justify-between items-center bg-white/50">
                <span className="text-sm font-black uppercase text-slate-800 tracking-widest">
                  Lộ trình chi tiết
                </span>
                <img
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/10 custom-scrollbar">
                {itineraryBlocks.length > 0 ? (
                  <div className="space-y-4">
                    ...
                    <div className="flex justify-between items-center px-2 pt-4">
                      <button
                        ...
                        className={`h-10 px-6 rounded-xl text-[10px] font-black transition-all ${
                          itiPage > 1
                  ...
                  <div className="h-full flex items-center justify-center p-12 text-center italic text-slate-400 text-xs">
                    Lộ trình chi tiết sẽ hiển thị tại đây sau khi bạn tạo.
                  </div>
                )}
              </div>
            </GlassCard>
