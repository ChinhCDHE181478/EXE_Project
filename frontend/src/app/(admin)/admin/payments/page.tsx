...
export default function AdminPaymentsPage() {
  ...
  const query = useMemo(() => {
    ...
    return params;
  }, [page, size, status, userId, startDate, endDate]);
  ...
  async function submitUpdate() {
    ...
    try {
      ...
      if (uAmount !== "") payload.amount = Number(uAmount);
  ...
  return (
    <div className="space-y-6">
      ...
      <Card
        title="Bộ lọc"
        desc="Lọc nhanh theo trạng thái, người dùng và thời gian."
      ...
      >
        {/* ✅ MOBILE LIST VIEW (không kéo ngang vẫn dùng đủ nút) */}
              ...
              {!loading &&
                data?.content?.map((p) => (
                  <tr key={p.id} className="hover:bg-[#0891b2]/5">
                    ...
                    <td className="px-4 py-3 text-slate-700">{p.orderCode}</td>
