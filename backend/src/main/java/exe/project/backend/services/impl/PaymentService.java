...
public class PaymentService implements IPaymentService {
    ...
    public PurchaseResponse createPurchase(PurchaseRequest request) throws Exception {
        ...
        SubscriptionPackage pkg = packageRepository
                ...
                .orElseThrow(() -> new RuntimeException("Package not found or inactive: " + request.getPackageCode()));
    ...
    public void handlePayOSCallback(Object webhookBody) throws Exception {
        ...
        // Nếu không tìm thấy payment (có thể là test webhook từ PayOS)
        if (payment == null) {
            log.warn(
                    "Payment not found for orderCode: {}. This might be a test webhook from PayOS. Skipping processing.",
                    orderCode);
            return;
        }
